...
void RunQueries()
{
    ...
    for (size_t nQ = 0; fgets(arrArgs[0], ffCountOf(arrArgs[0]), stdin); ++nQ)
    {
        ...
        if (s)
        {
            ...
            for (s->First(); !s->IsFinished() && nCount < g_nOutputLimit; s->Next(), ++nCount) 
            {
                ...
                printf("id:%7u\tpop:%10u\t%s" RN, 
                       *(unsigned*)g_Searcher.GetData(0, doc), 
                       doc.m.rank ? *(unsigned*)g_Searcher.GetData(1, doc) : 0, 
                       (const char*)g_Searcher.GetData(2, doc));
            }                        
